# Frozen Study Protocol (v1.3 — CONFIRMATORY FREEZE)
...
# PART III — FINAL CONFIRMATORY FREEZE (v1.3)

**Frozen 2026-09-15, before any confirmatory detector has been executed.** After this freeze no
endpoint, matching threshold, cohort rule, classification threshold, exclusion criterion,
statistical method or sensitivity analysis may be changed in response to confirmatory results.
The 24 development structures remain permanently exploratory and are never pooled into
confirmatory inference.

## III.1 [F1] Target-disjoint confirmatory set

Any confirmatory candidate whose TARGET UniProt accession occurs anywhere in the development set
is removed, even under a different fusion partner. Required target-accession overlap: **zero**.

**Final confirmatory n = 128** (1 entity removed from the 406-entity pool by F1; collapsed to one
structure per (partner, target) by highest resolution, ties by lowest PDB ID):

| Partner | n | Targets | Median res | Topology | Method |
|---|---|---|---|---|---|
| BRIL | 29 | 29 | 2.80 A | 17 internal, 10 N-term, 2 C-term | 24 X-ray, 5 cryo-EM |
| T4L | 38 | 38 | 2.70 A | 29 internal, 6 N-term, 2 C-term, 1 complex | 38 X-ray |
| MBP | 61 | 61 | 2.30 A | 57 N-term, 4 complex | 61 X-ray |
| **Total** | **128** | **128** | | | |

Frozen manifests: `data_manifest/development_set.*`, `data_manifest/confirmatory_set_final.*`.
Leakage test: `tests/test_no_development_leakage.py` — fails if any target accession, PDB entry,
polymer entity or (partner, target) pair occurs in both sets. Must pass before the confirmatory
run and again before reporting.

## III.2 [F2] The structure is the inferential unit

Individual matched pockets are **not** independent observations. Multiple cavities from one
structure are correlated, and the development analysis reported a Wilcoxon test over 364 individual
fpocket cavities as though n = 364. That is pseudoreplication and is withdrawn as inferential
evidence.

- **Primary inferential unit: one independent target / structure.** Every confirmatory
  hypothesis test, confidence interval and effect estimate uses n = number of structures.
- Cavity-level statistics may be reported **descriptively only**, always labelled as such, with
  the number of structures stated alongside the number of cavities.
- Any secondary cavity-level model must account for clustering by target/structure (random
  intercept or cluster-robust variance).

## III.3 [F3] Non-degenerate causal rank-displacement endpoint

Uses the frozen correspondence algorithm of II.1 unchanged (target residues only; min 3 target
residues; Jaccard; heavy-atom centroid distance; one-to-one assignment; MATCHED at Jaccard >= 0.40
AND centroid <= 8.0 A; rank and score never used for matching).

For each structure and each detector:

1. In the FUSION-REMOVED target-only structure, take the detector's **highest-ranked** predicted
   cavity.
2. Confirm it carries enough TARGET residues for correspondence (>= 3).
3. Match that same cavity into the ORIGINAL fusion-containing structure with the frozen algorithm.
4. Record `REMOVED_RANK`, `ORIGINAL_RANK`, and
   **`RANK_DISPLACEMENT = ORIGINAL_RANK - REMOVED_RANK`**.

Interpretation: **0** means the fusion did not displace the detector's intrinsically preferred
target cavity; **> 0** means the fusion pushed that same cavity downward.

Two states take no numerical rank and are reported separately:

- `DETECTOR_ABSTENTION` — the removed structure yielded no predictions.
- `TARGET_CAVITY_NOT_RECOVERED_IN_ORIGINAL` — the removed rank-1 cavity has no match in ORIGINAL.

Structure-level summaries: evaluable n, abstention n, unmatched n, median and mean displacement,
proportion displacement > 0, >= 2 and >= 5, bootstrap 95% CI, partner-stratified distributions.

## III.4 Final endpoint hierarchy

**PRIMARY AIM A — exposure.** A1: fusion-associated pocket at rank 1 in the ORIGINAL deposited
construct, per detector, reported as overall raw, BRIL, T4L, MBP, and a partner-standardised
estimate with weights 1/3 each, with Wilson and cluster-bootstrap CIs. A2: top-3 and top-5 as key
secondary rank-depth outcomes. Fusion-associated remains `FUSION_DOMINATED | INTERFACE | LINKER`
at dominance 0.70.

**PRIMARY AIM B — target-cavity rank displacement**, the F3 structure-level endpoint. This is the
primary evidence that the fusion *competes with* a target cavity rather than merely existing
alongside one. The old "rank-1 becomes TARGET_DOMINATED" outcome is retained in the audit trail
only and is never cited as causal evidence.

**SECONDARY AIM C — biological reference site.** Coverage determined before any detector output is
seen, by the frozen rules. If coverage >= 60% report prominently; if < 60% retain as a restricted
subset. Per structure: the detector pocket corresponding to the site, its original and removed
rank, rank change, whether any fusion-associated pocket outranks it, and top-1/3/5 recovery.

**SECONDARY AIM D — mechanism**, using the frozen isolated-partner correspondence method and the
frozen categories: `FUSION_INTRINSIC_CAVITY`, `TARGET_FUSION_INTERFACE`, `LINKER_RELATED`,
`UNDETERMINED_DETECTOR_NO_PREDICTIONS`, `NO_MATCHING_CAVITY`, `MAPPING_ARTIFACT`,
`CHIMERA_SPECIFIC_FUSION_CAVITY`. No new categories may be created after seeing confirmatory data.

**SECONDARY AIM E — detector dependence**: rank-1, top-3, top-5, displacement, abstention and
score margins for P2Rank vs fpocket, stratified by partner. Detector disagreement is a result and
is never hidden.

## III.5 Statistics — final freeze

1. Partner-stratified estimates are reported **first**, in the order BRIL, T4L, MBP, before any
   pooled number is interpreted.
2. Every overall exposure estimate appears **twice**: raw pooled and partner-standardised
   (1/3, 1/3, 1/3).
3. Structure-level rank displacement: median, empirical distribution, structure-level bootstrap
   CI, sign/direction counts, and a structure-level paired non-parametric test where inferential
   testing is retained. **No ordinary Wilcoxon over individual pockets.**
4. Secondary multivariable model (kept secondary): covariates fusion partner, topology, target
   size, resolution, experimental method, detector, segment disorder. **Check for complete or
   quasi-complete separation before fitting standard logistic regression**; on separation, use
   pre-specified Firth penalised logistic regression, or report that the conventional model is not
   estimable and give the robust alternative. Modelling strategy is never chosen by which yields
   smaller p-values. No interaction terms beyond **detector x partner**, which is scientifically
   motivated by the development results and tested only if estimable.
5. All confidence intervals and denominators are visible everywhere.

## III.6 Pre-specified sensitivity analyses (unchanged)

| Axis | Values |
|---|---|
| Pocket dominance | 0.50, **0.70 primary**, 0.90 |
| Correspondence | J>=0.25/12A, **J>=0.40/8A primary**, J>=0.60/5A, overlap coefficient >=0.50 |
| Deletion boundary | 8 A |
| E9 disorder | **0.20 primary**, 0.35 sensitivity, plus disorder as covariate |
| Structure representation | **single designated chain primary**, biological assembly 1 sensitivity |
| HETATM | **stripped primary**, ions-retained sensitivity |

No new sensitivity threshold may be added after seeing results unless explicitly labelled
post-hoc.

## III.7 Execution and result integrity

Full fresh cohort, ORIGINAL + FUSION-REMOVED, P2Rank 2.5.1 and fpocket source-tag 4.2.3, the exact
installed versions in `environment/env_lock.json`. No parameter tuning, no per-structure rescue.
Every run records command, version, input hash, output hash, stdout/stderr, exit code and runtime.
Technical failures remain in the manifest and are never silently substituted.

Once the confirmatory run begins, scientific rules do not change. If an unforeseen technical defect
invalidates the analysis: **stop**, document it, state whether any outcome was already visible, and
request external review before changing methodology. A pure implementation bug that demonstrably
does not depend on outcome may be corrected only with a before/after test, an explicit DEVIATIONS
entry, and a statement of whether results had already been exposed.

## III.8 [F4] Within-confirmatory clustering: 128 structures, 123 independent target clusters

Five target accessions appear twice in the confirmatory set, each time as a **cross-partner pair**
of the same receptor: beta-2 adrenergic (8JJ8 BRIL / 2RH1 T4L), M2 muscarinic (5ZKC / 3UON),
adenosine A2A (5NM4 / 3EML), glucagon receptor (4L6R / 5EE7), and CRF receptor 1 (3EHS MBP /
4K5Y T4L). No target is duplicated under the same partner.

These are retained, for two reasons: they are matched within-target contrasts of two different
fusion partners on the identical protein, which is scientifically informative for Aim E; and the
statistical plan already clusters on target accession, so their correlation is modelled rather
than ignored.

Consequently **every reported quantity states both n_structures = 128 and n_independent_target
clusters = 123**, and all confidence intervals use the clustered bootstrap on target accession.
The leakage test asserts that any duplicated target is a legitimate cross-partner pair
(`tests/test_no_development_leakage.py`, check L6).
