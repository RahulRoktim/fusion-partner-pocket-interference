# Claims Ledger — manuscript branch, v2

**This is a derived file. It does not replace `results/CLAIMS_LEDGER.md`, which remains frozen and
is hashed in `environment/FINAL_FREEZE_v1.1.json`.**

This copy is byte-identical to the frozen ledger except for one field, corrected here and recorded
in the table below. No claim, cohort, numerator, denominator, permitted wording or prohibited
overstatement is altered. No scientific value changed.

| Change | Claim | Frozen ledger | This file | Source of the correction |
|---|---|---|---|---|
| 1 | C4, STATISTICAL SUPPORT | `P2Rank p = None, fpocket p = None` | `P2Rank 42 up / 0 down p = 4.55e-13, fpocket 65 up / 0 down p = 5.42e-20` | `results/confirmatory/primary/confirmatory_summary.txt` lines 73, 91; `results/CONFIRMATORY_REPORT.md` PRIMARY AIM B |

The `None` values in the frozen ledger are a value-injection gap in the ledger-generating script,
not a disagreement between sources: the confirmatory report and the frozen summary both carry the
p-values explicitly. `MANUSCRIPT_DRAFT_v2.md` uses the confirmatory-report values, reported as
subordinate to the direction of the effect and to the absence of counter-examples.

**Governance.** The generating script should be corrected in the manuscript branch and the ledger
regenerated as a separately hashed artifact. `results/CLAIMS_LEDGER.md`, the v1.0 and v1.1 tags and
`environment/FINAL_FREEZE_v1.1.json` must not be modified.
...
- **DETECTOR:** both
- **STATISTICAL SUPPORT:** Structure-level exact sign test, P2Rank 42 up / 0 down p = 4.55e-13, fpocket 65 up / 0 down p = 5.42e-20; cluster-bootstrap intervals. ONE STRUCTURE = ONE OBSERVATION. *(Values populated in the manuscript branch from `results/confirmatory/primary/confirmatory_summary.txt` lines 73 and 91, also printed in `results/CONFIRMATORY_REPORT.md`; the frozen ledger renders these fields as `None` because of a value-injection gap in `scripts/33_claims_ledger.py`. No value changed.)*
